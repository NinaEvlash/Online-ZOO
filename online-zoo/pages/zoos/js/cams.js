const animalPageArray = [
  {
    name: "panda",
    title: "live panda cams",
    image: "../../assets/images/youtube-player-panda.png",
    href: "#",
    slider: [
      {
        nameCam: "Cam one",
        imgCam: "../../assets/images/panda-cam1.png",
        hrefCam: "#",
      },
      {
        nameCam: "Cam two",
        imgCam: "../../assets/images/panda-cam2.png",
        hrefCam: "#",
      },
      {
        nameCam: "Cam three",
        imgCam: "../../assets/images/panda-cam3.png",
        hrefCam: "#",
      },
    ],
  },
...
const panelAnimalArray = [
  { name: "Panda", icon: "../../assets/icons/Panda.png", link: "#" },
  { name: "Eagle", icon: "../../assets/icons/Eagle.png", link: "#" },
  { name: "Gorilla", icon: "../../assets/icons/Gorilla.png", link: "#" },
  { name: "Lemur", icon: "../../assets/icons/Lemur.png", link: "#" },
];
...
function createSliderButton(type, customClass) {
  ...
  button.innerHTML = spanArrow;

  return button;
}

function createCamsSection(animal) {
...
function createLeftPanel(animalArray) {
  const panelContainer = document.createElement("div");
  panelContainer.classList.add("cams__panel", "flex");

  const topBtn = document.createElement("button");
  topBtn.classList.add("cams__panel__top-btn");
  topBtn.innerHTML = `
  <svg width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M7 13.6591L8.36504 15L16 7.5L8.36504 0L7 1.34091L13.2699 7.5L7 13.6591Z" fill="white"/>
  <path d="M0 13.6591L1.36504 15L9 7.5L1.36504 0L0 1.34091L6.26992 7.5L0 13.6591Z" fill="white"/>
  </svg>
  `;

  const panelLink = document.createElement("ul");
  panelLink.classList.add("cams__panel__list", "reset-list");
  ...
  animalArray.forEach((animal) => {
    ...
    image.classList.add("cams__panel__item-img");
    image.src = animal.icon;
    image.alt = animal.name;

    link.append(image);
    item.append(link);
    panelLink.append(item);
  });

  const buttonBtn = document.createElement("button");
  buttonBtn.classList.add("cams__panel__button-btn");
  buttonBtn.innerHTML = `
  ...
  const panelTopContent = document.createElement("div");
  panelTopContent.classList.add("cams__panel__top-content");

  const spanText = document.createElement("span");
  spanText.textContent = "LIVE";
  const spanIcon = document.createElement("span");
  spanIcon.innerHTML = `
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M10.0051 10.9923C10.6159 10.9923 11.2128 10.8162 11.7206 10.4863C12.2284 10.1563 12.6242 9.68741 12.8579 9.13877C13.0916 8.59013 13.1527 7.98642 13.0336 7.40398C12.9145 6.82155 12.6204 6.28654 12.1885 5.86663C11.7567 5.44672 11.2065 5.16075 10.6075 5.0449C10.0086 4.92904 9.38772 4.98851 8.8235 5.21576C8.25929 5.44302 7.77704 5.82786 7.43775 6.32162C7.09846 6.81539 6.91737 7.3959 6.91737 7.98975C6.91737 8.78607 7.24268 9.54978 7.82175 10.1129C8.40083 10.676 9.18622 10.9923 10.0051 10.9923ZM10.0051 6.9889C10.2087 6.9889 10.4077 7.0476 10.577 7.15757C10.7462 7.26755 10.8782 7.42386 10.9561 7.60674C11.034 7.78962 11.0543 7.99086 11.0146 8.185C10.9749 8.37915 10.8769 8.55748 10.7329 8.69745C10.589 8.83742 10.4056 8.93275 10.2059 8.97136C10.0063 9.00998 9.79934 8.99016 9.61127 8.91441C9.42319 8.83866 9.26244 8.71038 9.14935 8.54579C9.03625 8.3812 8.97589 8.1877 8.97589 7.98975C8.97589 7.72431 9.08433 7.46974 9.27735 7.28204C9.47037 7.09435 9.73217 6.9889 10.0051 6.9889ZM19.8758 16.1567C19.3732 14.5358 18.456 13.0654 17.21 11.883C17.9035 10.6641 18.2581 9.29024 18.2385 7.89726C18.2189 6.50429 17.8258 5.14039 17.0982 3.94032C16.3706 2.74026 15.3336 1.74557 14.0896 1.05452C12.8457 0.363479 11.4378 0 10.0051 0C8.57249 0 7.16463 0.363479 5.92066 1.05452C4.67669 1.74557 3.63967 2.74026 2.91206 3.94032C2.18445 5.14039 1.79143 6.50429 1.77183 7.89726C1.75224 9.29024 2.10674 10.6641 2.80032 11.883C1.547 13.0615 0.625601 14.533 0.124247 16.1567C-0.0097316 16.6019 -0.0357768 17.0713 0.0481558 17.528C0.132088 17.9847 0.323715 18.4163 0.608 18.7889C0.897643 19.1677 1.27506 19.4748 1.70963 19.6852C2.14419 19.8956 2.62366 20.0035 3.1091 19.9999H16.9012C17.3849 20.0019 17.8623 19.8933 18.295 19.683C18.7276 19.4726 19.1034 19.1664 19.392 18.7889C19.6763 18.4163 19.8679 17.9847 19.9518 17.528C20.0358 17.0713 20.0097 16.6019 19.8758 16.1567ZM10.0051 1.98466C11.2266 1.98466 12.4205 2.33686 13.4361 2.9967C14.4517 3.65655 15.2432 4.59442 15.7106 5.6917C16.178 6.78899 16.3003 7.99641 16.062 9.16128C15.8238 10.3262 15.2356 11.3962 14.3719 12.236C13.5083 13.0758 12.4079 13.6477 11.2099 13.8794C10.012 14.1112 8.7703 13.9922 7.64186 13.5377C6.51342 13.0832 5.54893 12.3135 4.87035 11.326C4.19177 10.3385 3.82958 9.17744 3.82958 7.98975C3.82958 6.3971 4.48022 4.86968 5.63836 3.74351C6.79651 2.61734 8.36729 1.98466 10.0051 1.98466ZM17.7452 17.5879C17.6483 17.7168 17.5212 17.8213 17.3744 17.8926C17.2276 17.964 17.0653 18.0002 16.9012 17.9982H3.1091C2.94497 18.0002 2.78272 17.964 2.6359 17.8926C2.48908 17.8213 2.36194 17.7168 2.26511 17.5879C2.17256 17.4639 2.11051 17.3209 2.08375 17.17C2.05698 17.019 2.06621 16.8641 2.11072 16.7171C2.48019 15.5074 3.14391 14.4019 4.04573 13.4944C4.81469 14.2786 5.73935 14.9029 6.76371 15.3295C7.78807 15.7561 8.89081 15.976 10.0051 15.976C11.1195 15.976 12.2222 15.7561 13.2466 15.3295C14.2709 14.9029 15.1956 14.2786 15.9646 13.4944C16.8664 14.4019 17.5301 15.5074 17.8996 16.7171C17.9441 16.8641 17.9533 17.019 17.9265 17.17C17.8998 17.3209 17.8377 17.4639 17.7452 17.5879Z" fill="white"/>
  </svg>
  `;

  panelTopContent.append(spanText, spanIcon);

  panelContainer.append(topBtn, panelTopContent, panelLink, buttonBtn);

  return panelContainer;
}

const camsSection = document.getElementById("cams-panda-section");
