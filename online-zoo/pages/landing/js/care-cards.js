...
function createCareCard(care, index) {
  ...
  cardImage.classList.add("card-top-img");

  cardImage.src = care.image;
  cardImage.alt = "Animal foto";

  cardImageContainer.append(cardImage);

  const descr = document.createElement("p");
  descr.classList.add("card-care-text", "section-text");

  descr.textContent = care.text;

  const button = document.createElement("div");
  button.classList.add("card-care-button", "section-btn");

  const textBtn = document.createElement("span");
  const iconBtn = document.createElement("span");

  textBtn.textContent = "FEED";
  iconBtn.innerHTML = `
